...
function renderAI() {
  ...
  const messagesHtml = aiChatHistory.length
    ? aiChatHistory.map((entry) => `
      <div class="rounded-xl p-4 ${entry.role === "user" ? "bg-accent-primary/10 border border-accent-primary/20" : "bg-surface-container-low border border-border-default"}">
        <p class="text-[11px] uppercase tracking-wider ${entry.role === "user" ? "text-accent-primary" : "text-accent-secondary"}">${entry.role === "user" ? "You" : "BudgetIT AI"}</p>
        <p class="text-sm text-text-primary mt-1 whitespace-pre-wrap">${escapeHtml(entry.content)}</p>
      </div>
    `).join("")
    : `
      <div class="rounded-xl p-5 bg-surface-container-low border border-border-default">
        <p class="text-sm text-text-secondary">Ask anything about your budget, savings plan, or spending habits. Example: \"How can I reduce my food spending this week?\"</p>
      </div>
    `;
  ...
  <div class="flex items-center justify-between">
    <div>
      <h1 class="text-[28px] font-bold font-headline">AI Assistant</h1>
      <p class="text-text-secondary mt-1">Ask questions and get tailored guidance for your finances.</p>
    </div>
    <button type="button" onclick="clearAIChat()" class="btn-ghost h-10">
      <span class="material-symbols-outlined text-[18px]">delete</span>Clear Chat
    </button>
  </div>

  <div class="card space-y-4">
    <div id="ai-chat-messages" class="space-y-3 max-h-[460px] overflow-y-auto pr-1">${messagesHtml}</div>
    <form id="ai-chat-form" class="flex gap-3 items-end" onsubmit="sendAIQuestion(event)">
      <div class="flex-1">
        <label class="input-label">Ask your question</label>
        <textarea id="ai-question" class="input-field w-full min-h-[100px] resize-y" placeholder="Type your question..." required style="background-color:#12121A;color:#F0F0F5"></textarea>
      </div>
      <button type="submit" id="ai-send-btn" class="btn-primary h-11">
        <span class="material-symbols-outlined text-[18px]">send</span>Ask AI
      </button>
    </form>
  </div>`;
}
...
function clearAIChat() {
  aiChatHistory = [];
  if (currentPage === "ai") renderAI();
}

async function sendAIQuestion(event) {
    ...
    const response = await apiRequest("ai", {
      ...
      body: {
        ...
        context: {
          ...
          goals: state.savingsGoals.map((g) => ({ name: g.name, target: g.target, saved: g.saved })),
        },
      },
    });

    const aiText = response && response.reply ? String(response.reply) : "I could not generate a response just now.";
    aiChatHistory.push({ role: "assistant", content: aiText });
  } catch (_err) {
    aiChatHistory.push({ role: "assistant", content: "I couldn't reach the AI service right now. Please try again in a moment." });
  } finally {
